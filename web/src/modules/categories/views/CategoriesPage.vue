  ...
  <Root>
    <div class="flex flex-col gap-5 h-full min-w-0 p-8 overflow-auto">
      ...
      <div v-if="isLoading" class="grid grid-cols-2 gap-4">
        <div v-for="i in 8" :key="i" class="bg-[#131316] border border-[#26262c] rounded-[14px] p-5 h-32 animate-pulse" />
      </div>
      ...
      <div v-else class="grid grid-cols-2 gap-4">
        <div
          ...
          :key="cat.name"
          class="bg-[#131316] border border-[#26262c] rounded-[14px] p-5 flex flex-col gap-3"
        >
          <div class="flex items-center gap-3">
            ...
            >
              <i :class="cat.icon" class="text-[16px]" />
            </div>
            <div class="flex-1 min-w-0">
              <p class="text-white font-semibold text-sm leading-tight">{{ cat.name }}</p>
              <p class="text-zinc-400 text-xs mt-0.5">Budget: {{ fmt(cat.budget) }}</p>
            </div>
            <div class="text-right">
              <p class="text-white font-bold text-base tabular-nums">{{ fmt(cat.spent) }}</p>
              <p class="text-[11px] mt-0.5" :class="cat.remaining >= 0 ? 'text-emerald-400' : 'text-red-400'">
          ...
          <div class="flex flex-col gap-1">
            <div class="h-1.5 rounded-full bg-[#26262c] overflow-hidden">
              <div
            ...
            <p class="text-[11px] text-zinc-500">{{ cat.pct }}% of budget used</p>
          </div>
        </div>
      </div>
    </div>
